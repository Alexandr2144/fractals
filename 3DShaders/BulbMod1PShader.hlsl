...
float3 trace(float3 StartPos,float3 Step)
{
	...
	for(int Count = 0; Count < 1000; Count++)
	{
		...
		for(int i = 0; i < 12; i++){
			if(dot(Z,Z) > 4) break;
			tZ.x = pow(Z.x,5)-10*pow(Z.x,3)*dot(Z.yz,Z.yz)+5*Z.x*(pow(Z.y,4)+pow(Z.z,4))+Ray.x;
			tZ.y = pow(Z.y,5)-10*pow(Z.y,3)*dot(Z.xz,Z.xz)+5*Z.y*(pow(Z.x,4)+pow(Z.z,4))+Ray.y;
			tZ.z = pow(Z.z,5)-10*pow(Z.z,3)*dot(Z.xy,Z.xy)+5*Z.z*(pow(Z.x,4)+pow(Z.y,4))+Ray.z;
			Z = tZ;
		}
		if(dot(Z,Z) < 4) return Ray;
		Ray += Step;
	}
